...
class VISORFormatter:
    ...
    def generate_davis(self):
        ...
        # for subseq in self.data.get_subsequence():
            ...
            for idx, frame in enumerate(subseq['frames']['sparse']):
                ...
                mask, object_to_color, raw_info = self._process_sparse_mask(subseq_name, frame, object_to_color)
            ...
            for idx, frame in enumerate(combined_frames):
                ...
                # Write which frame is sparse to file
        ...
        # Print stats
        if self.augment:
            print("Stats for refined {} set with dense frames".format(self.subset))
        else:
            print("Stats for refined {} set".format(self.subset))
        print("Total subsequences: {}".format(self.stats['num_valid_subseqs']))
        print("Total frames: {}".format(self.stats['num_frames']))
        print("Total objects: {}".format(self.stats['num_entities']))
        print("-" * 20)
    ...
    def _process_sparse_mask(self, subseq_name, frame, object_to_color):
        # Get the next color, start from 1
        mask = np.zeros((1080, 1920), dtype=np.uint8)
        color = sorted(object_to_color.values())[-1] + 1 if object_to_color else 1
        ...
        for side in ['left', 'right']:
            if (object_id := frame[f'{side} object'][0]): # there should be only one value here
                ...
                if self.include_hand: # add hand polygons into mask
                    hand_color = object_to_color.get(f'{side} hand', color)
                ...
                object_color = object_to_color.get(object_name, color)
                ...
                color = max(color, object_color + 1)
...
if __name__ == '__main__':
    ...
    out_root = '.'
    data_formatter = VISORFormatter(data_config, out_root, augment=False, include_hand=True)
