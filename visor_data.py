...
class VISORData:
    """
    VISOR Data Class, for preprocessing data.
    NOT PyTorch Dataset class.

    Subsequences is stored in a dictionary, with key is subsequence name.
    """
    ...
    GLOVE_VALID_VALUES: list = [['right hand'],
                          ['left hand'],
                          ['left hand', 'right hand']] # Exclude 'inconclusive', None
    ...
    def __init__(self,
        ...
        self.sparse_img_dir = os.path.join(self.sparse_root, 'rgb_frames', self.subset)
        self.sparse_anno_dir = os.path.join(self.sparse_root, 'annotations', self.subset)

        # Unpack config
        self.filter_mode = config['coverage_filter']['mode']
        ...
        if self.dense_root:
            ...
            self.dense_anno = (None, {}) # current dense annotation (json file), because it's so huge, can not load all at once
            self.desired_fps = config['dense_sample']['fps'] # sample rate for Dense augmentation
            self.dense_img_dir = dense_img_dir

        self._check_directories()
        self._load_subsequences()

    def get_subsequence(self, augment: bool = False) -> dict:
        """Function to get pre-processed subsequence from VISOR dataset

        Args:
            augment (bool, optional): add dense annotation to subsequence. Defaults to False.

        Yields:
            Iterator[dict]: a dictionary of []
        """
    ...
    def get_subsequence_names(self) -> list:
        """Return all subsequence names from the dataset

        Returns:
            list: names of subsequences
        """
    ...
    def _check_directories(self) -> None:
        ...
        if not os.path.exists(self.sparse_root):
            raise FileNotFoundError(f'VISOR Sparse not found in the {self.sparse_root} directory')

        if not os.path.exists(self.sparse_img_dir) or not os.path.exists(self.sparse_anno_dir):
            raise FileNotFoundError(f'VISOR Sparse subset {self.subset} not found')

        if self.dense_root:
            if not os.path.exists(self.dense_root):
                raise FileNotFoundError(f'VISOR Dense Annotations not found in the {self.dense_root} directory')

            if not os.path.exists(self.dense_img_dir):
                raise FileExistsError(f'VISOR Dense Frames not found in {self.dense_img_dir} directory')

    def _load_subsequences(self) -> None:
        """
        Load sparse annotation .json file and group frames into self.subsequence
        Each frame is a dictionary of:
            name: id of the frame, in format Pxx_xxx_frame_xxxxxxxxxx
            subsequence: subsequence the frame belongs to
            image_path: path to the jpg image
            annotations: list of object annotations in the frame (raw, unprocessed)
        ...
        for json_file in tqdm(sorted(glob.glob(os.path.join(self.sparse_anno_dir, '*.json')))):
            ...
            for frame in data['video_annotations']: # Loop over frames in a sequence
                ...
                if 'P04_121_frame_0000075601' in frame['image']['name']:
                    for entity in frame['annotations']:
                        if entity['id'] == '6e234d3c52ecafb034c199642f6373eb':
                            entity['on_which_hand'][-1] = 'right hand' # typo error: rigth
                ...
                frame['image_path'] = os.path.join(self.sparse_img_dir,
                ...
                del frame['image'] # delete the image raw metadata
    ...
    def _filter_object(self, entity: dict) -> bool:
        ...
        Args:
            entity (dict): object annotation

        Returns:
            bool: True if object is passed the filter, False otherwise
        """
    ...
    def _process_sparse_frame(self, frame: dict) -> None:
        ...
        Args: frame (dict): frame annotatation and metadata

        Returns: sparse frame (dict) with additional data
        ...
        entities = {}
        for entity in frame['annotations']:
            entity['coverage'] = calculate_segment_area(entity['segments'], self.resolution, self.filter_mode) / np.prod(self.resolution)
        ...
        for entity_id, entity in entities.items():
            ...
            if entity['class_id'] in VISORData.HAND_IDS:
                ...
                check_object = (object_id not in VISORData.INVALID_CONTACTS # only valid contacts has object_id
                ...
                if check_object:
                    ...
                    check_glove = ('on_which_hand' in entities[object_id]
                                    and entities[object_id]['on_which_hand'] in VISORData.GLOVE_VALID_VALUES # have valid values
                                    and entity['name'] in entities[object_id]['on_which_hand']) # hand itself is in list of glove's on_which_hand

                    if not check_glove:
                        if self._filter_object(entities[object_id]):
                            # hand contacts with a valid and not glove objects, update frame metadata
                            frame.setdefault('relations', []).append((entity_id, object_id))
                            # add to left object or right object field
            ...
            elif 'on_which_hand' in entity and entity['on_which_hand'] in VISORData.GLOVE_VALID_VALUES:
    ...
    def _load_dense_annotation(self, sequence_name: str) -> None:
        """
        Load dense annotation based on input sequence name to class.
        Json dense annotation takes time to load so we only load it when needed.
        Note: In EPIC-KITCHEN, Pxx is video name and Pxx_xxx is clip name.
        Args:
            sequence_name (str): sequence name in format Pxx_xxx_seq_xxxxx
        """
        dense_anno_template = os.path.join(self.dense_root, self.subset,
                                           '{}_interpolations', '{}_interpolations.json')

        clip_name = sequence_name.split('_seq_')[0] # Pxx_xxx
        if clip_name != self.dense_anno[0]: # if this dense anno is not loaded
            json_file = dense_anno_template.format(clip_name, clip_name)
            ...
            for frame in data['video_annotations']:
                video_name = frame['image']['image_path'].split('_')[0] # Pxx
                frame['image_path'] = os.path.join(self.dense_img_dir, self.subset, video_name, frame['image']['image_path'].replace('.png', '.jpg'))
                ...
                frame['interpolation'] = frame['image']['interpolation']
            ...
            self.dense_anno = (clip_name, dense_frames)
